...
func CreateRecipe(ctx types.TestContext, recipeSpec *types.Recipe) error {
	log := ctx.Logger()

	recipeC1 := getBaseRecipe(ctx)
	ns := ctx.AppNamespace()

	if recipeSpec.ExecuteCheckHooks || recipeSpec.ExecuteExecHooks {
...
func getWorkflows(recipeSpec *types.Recipe) []*recipe.Workflow {
	...
	if recipeSpec.ExecuteExecHooks && recipeSpec.ExecuteCheckHooks {
	...
	if recipeSpec.ExecuteCheckHooks {
	...
	if recipeSpec.ExecuteExecHooks {
