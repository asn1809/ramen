...
func EvaluateCheckHook(client client.Client, hook *kubeobjects.HookSpec, log logr.Logger) (bool, error) {
	...
	case "pod":
		...
		return EvaluateCheckHookExpWithTimeout(client, resource, nsScopedName,
			time.Duration(timeout)*time.Second, hook.Chk.Condition)
	case "deployment":
...
func EvaluateCheckHookExpWithTimeout(client client.Client, obj client.Object, nsScopedName types.NamespacedName,
	...
	for {
		select {
		case <-ctx.Done():
			return false, fmt.Errorf("timeout waiting for resource %s to be ready: %w", nsScopedName.Name, ctx.Err())
		case <-ticker.C:
